...
megaelixer = Item("MegaElixer", "elixer", "Fully restores party's HP/MP points", 9999)
...
while running:
    ...
    for player in players:
        ...
        if index == 0:
            ...
            if enemies[enemy].get_hp() == 0:
                print(enemies[enemy].name.replace(" ", "") + "has dies")
                del enemies[enemy]
        elif index == 1:
            player.choose_magic()
    ...
    for enemy in enemies:
        ...
        if enemy_choice == 0:
            ...
            print(enemy.name.replace(" ", "") + "choose", spell, " damage is ", magic_damage)
            enemy.reduce_mp(spell.cost)

            if spell.type == "white":
                enemy.heal(magic_damage)
                print(bcolors.OKBLUE + spell.name + "heals" + enemy.name.replace(" ", "") + "for", str(magic_damage), "HP." + bcolors.ENDC)
            elif spell.type == "black":
                target = random.randrange(0, len(players))
                players[target].take_damage(magic_damage)

                print(bcolors.OKBLUE + "\n" + enemy.name.replace(" ", "") + "'s"+ spell.name + " deals", str(magic_damage), "points of damage to " +
                      players[target].name.replace(" ", "") + bcolors.ENDC)
                if players[target].get_hp() == 0:
                    print(players[target].name.replace(" ", "") + "has dies")
                    del players[target]
